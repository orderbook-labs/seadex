...
use crate::{
    msg::{CurrentStateResp, OwnerResp, PoolsResp, QueryMsg},
    state::{OWNER, POOLS, STATE},
};
...
pub fn lotteries(deps: Deps) -> StdResult<Binary> {
    let lotteries: StdResult<Vec<_>> = POOLS
        .range(deps.storage, None, None, Order::Ascending)
        .collect();
    let lotteries = lotteries?.into_iter().map(|(_, lottery)| lottery).collect();
    to_binary(&PoolsResp { lotteries })
}
